...
MODEL_ID = "UPB-RAT-VLA/pi05_summerschool"
HF_TOKEN = None  # For private repos
CHUNK_EXEC_LEN = 50
ACTION_IDXS = [0, 2, 5]
SEED = 42
NUM_EVAL_STEPS = 20  # Number of completed episodes (done=True) before stopping
# =========================================================
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    # eval_count   = 0
    # success_count = 0  # 💡 NEW: Track successful episodes
    # episode_step = 0
    ...
    #         task_names = env.unwrapped.get_task_names()
    #         TASK = task_names[0]
    #         print("Env 0 task:", task_names[0])
    ...
    #         # 💡 CHANGE: Extract 'info' from the step function
    #         obs, _, done, info = env.step(extracted_action)

    #         # 💡 CHANGE: Synchronize CUDA to guarantee accurate env_fps measurements
    ...
    #             # 💡 NEW: Extract final distance metric from info logs safely
    #             final_dist = None
    #             if isinstance(info, dict):
    #                 if "log" in info and isinstance(info["log"], dict):
    #                     final_dist = info["log"].get("Metrics/final_distance_to_goal", None)
    #                 else:
    #                     final_dist = info.get("Metrics/final_distance_to_goal", None)

    #             # 💡 NEW: Process tensor scalars if necessary
    #             if isinstance(final_dist, torch.Tensor):
    #                 final_dist = final_dist.item()

    #             # 💡 NEW: Evaluate victory condition (< 1.2 meters)
    #             if final_dist is not None and final_dist < 1.2:
    #                 success_count += 1
    #                 print(f"[EVAL] Episode done — SUCCESS! 🏆 (Final Dist: {final_dist:.2f}m) | Completed: {eval_count} / {NUM_EVAL_STEPS}")
    #             else:
    #                 dist_str = f"{final_dist:.2f}m" if final_dist is not None else "Unknown"
    #                 print(f"[EVAL] Episode done — FAILED. ❌ (Final Dist: {dist_str}) | Completed: {eval_count} / {NUM_EVAL_STEPS}")
    ...
    # # 💡 NEW: Show absolute summary at the conclusion of evaluation script
    # success_rate = (success_count / eval_count * 100) if eval_count > 0 else 0.0
    # print(f"[EVAL] Done — completed {eval_count} / {NUM_EVAL_STEPS} evaluations over {total_steps} steps.")
    # print(f"[📊 STATS] Total Successes: {success_count} / {eval_count} | Success Rate: {success_rate:.2f}%")
    # env.close()
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            # 💡 NEW: Update the distance ALWAYS at the start of the step
            current_dist = (raw_obs["rel_px"]**2 + raw_obs["rel_py"]**2 + raw_obs["rel_pz"]**2) ** 0.5
            ...
            if done.any():
                ...
                # 💡 NEW: Check the saved distance value right before this reset step completed
                if current_dist < 1.2:
                    success_count += 1
                    print(f"[EVAL] Episode done — SUCCESS! 🏆 (Final Dist: {current_dist:.2f}m) | Completed: {eval_count} / {NUM_EVAL_STEPS}")
                else:
                    print(f"[EVAL] Episode done — FAILED. ❌ (Final Dist: {current_dist:.2f}m) | Completed: {eval_count} / {NUM_EVAL_STEPS}")

                if eval_count >= NUM_EVAL_STEPS:
                    break
                episode_step = 0
                current_dist = float("inf") # Reset local tracking variable for next episode
